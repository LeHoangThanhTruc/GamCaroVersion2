    ...
    public partial class Form1 : Form
    {
        ...
        void Listen()
        {
            Thread listenThread = new Thread(() =>
            {
                try
                {
                    SocketData data = socket.Receive<SocketData>();
                    if (data != null)
                        HandlingData(data);
                }
                catch (Exception ex)
        ...
        private void HandlingData(SocketData data)
        {
            switch (data.Command)
            {
                ...
                case (int)SocketCommand.END_GAME:
                    MessageBox.Show("Player had five in a line!","Result",MessageBoxButtons.OK,MessageBoxIcon.Information);
                    break;
                ...
                case (int)SocketCommand.QUIT:
                    ...
                    break;
                case (int)SocketCommand.ASSIGN_PLAYER:
                    ChessBoard.CurrentPlayer = 1;//int.Parse(data.Message); // 1 hoặc 2
                    pnlChessBoard.Enabled = (ChessBoard.CurrentPlayer == 1); // player1 đi trước
                    txtPlayerName.Text = "Player " + ChessBoard.CurrentPlayer;
                    ChessBoard.MyPlayerNumber = int.Parse(data.Message); // 1 hoặc 2
                    MessageBox.Show("Your turn is " + ChessBoard.MyPlayerNumber + "\n(1 : playing first,2 : playing second)");
                    break;
                default:
                    break;
            }
        ...
        private void btnConnect_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Cannot connect to server. Please check IP or start server first.");
                return;
            }

            pnlChessBoard.Enabled = false; // chờ server gửi thông tin lượt
            Listen(); // lắng nghe dữ liệu từ server

            txtPlayerName.Text = "Waiting...";
